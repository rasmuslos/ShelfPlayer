...
public final class CarPlayDelegate: UIResponder {
    ...
    private var connectionChangedSubscription: AnyCancellable?

    override init() {
        super.init()

        connectionChangedSubscription = PersistenceManager.shared.authorization.events.connectionsChanged
            .sink { [weak self] _ in
