//
//  GroupingConfigurationSheet.swift
//  ShelfPlayer
//
                ...
                if let viewModel {
                    ...
                    Section {
                        if viewModel.isUpNextCustomizable {
                            ...
                            .disabled(!settings.generateUpNextQueue)
                        }
                    ...
                    if viewModel.areConvenienceDownloadsAvailable {
                        ...
                        .disabled(!settings.enableConvenienceDownloads)
                    }
                } else {
                    LoadingView.Inner()
                        .task {
...
private final class ViewModel {
    ...
    private var defaultPlaybackRate: Double { AppSettings.shared.defaultPlaybackRate }

    init(itemID: ItemIdentifier) async {
        self.itemID = itemID

        playbackRate = await PersistenceManager.shared.item.playbackRate(for: itemID) ?? AppSettings.shared.defaultPlaybackRate
...
enum ConvenienceDownloadRetrievalOption: String, Identifiable {
    ...
    var resolved: PersistenceManager.ConvenienceDownloadSubsystem.GroupingRetrieval? {
        ...
        case .five: .amount(5)
        case .ten: .amount(10)
        case .oneDay: .cutoff(24)
        case .oneWeek: .cutoff(168)
        case .twoWeeks: .cutoff(336)
        case .oneMonth: .cutoff(672)
        case .all: .all
        }
    }
    ...
    static func options(for itemType: ItemIdentifier.ItemType) -> [[Self]] {
        switch itemType {
            case .podcast:
                [[.disabled], [.one, .two, .three, .four, .five, .ten], [.oneDay, .oneWeek, .twoWeeks, .oneMonth], [.all]]
            case .series, .collection, .playlist:
                [[.disabled], [.one, .two, .three, .four, .five, .ten], [.all]]
            default:
                [[.disabled]]
        }
    }
