    ...
    func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didFinishDownloadingTo location: URL) {
        ...
        Task.detached { @MainActor [self] in
            if let reference = OfflineManager.shared.getReferenceByDownloadTaskId(downloadTask.taskIdentifier) {
                if let track = OfflineManager.shared.getAudiobook(trackId: reference.reference) {
                    ...
                    do {
                        ...
                        try? FileManager.default.removeItem(at: tmpLocation)
                        try? OfflineManager.shared.delete(audiobookId: track.audiobookId)
                        
                        logger.fault("Error while moving track \(track.id): \(error.localizedDescription)")
                    }
                    
                    return
                } else if let episode = OfflineManager.shared.getEpisode(episodeId: reference.reference) {
